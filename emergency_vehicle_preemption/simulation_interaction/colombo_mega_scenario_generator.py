...
class MegaScenarioGenerator:
    def __init__(self, filename_base="colombo_mega_scenario"):
        self.filename_base = filename_base
        self.net_file = os.path.join(NET_DIR, "colombo.net.xml") 
    ...
    def generate_network(self):
        """Downloads Rajagiriya-Malabe OSM data and converts to SUMO Network."""
        print("1. Generating Real-World Network (Rajagiriya to Malabe)...")
        
        osm_file = os.path.join(NET_DIR, "colombo.osm")
        self.net_file = os.path.join(NET_DIR, "colombo.net.xml")

        # Bounding Box for Rajagiriya -> Malabe (Optimized)
        # South: 6.900, North: 6.925, West: 79.885, East: 79.970
        min_lat, min_lon = 6.900, 79.885
        max_lat, max_lon = 6.925, 79.970
        
        if not os.path.exists(osm_file):
            print("   Downloading OSM Data (via Overpass API)...")
            # Use Overpass API instead of Main API to avoid 50k node limit
            ...
            try:
                ...
                print("   Trying backup mirror...")
                try:
                    url_backup = f"https://lz4.overpass-api.de/api/map?bbox={min_lon},{min_lat},{max_lon},{max_lat}"
                    urllib.request.urlretrieve(url_backup, osm_file)
                    print("   Download Complete (Backup Mirror).")
