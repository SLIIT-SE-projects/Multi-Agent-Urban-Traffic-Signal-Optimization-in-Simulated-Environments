...
class KatunayakeScenarioGenerator:
    def __init__(self, filename_base="katunayake"):
        self.filename_base = filename_base
        self.net_file = os.path.join(NET_DIR, "katunayake.net.xml") 
    ...
    def generate_network(self):
        """Converts Katunayake OSM data to SUMO Network."""
        print("1. Generating Real-World Network (Katunayake)...")
        
        osm_file = os.path.join(NET_DIR, "katunayake.osm")
        
        if not os.path.exists(osm_file):
            print(f"CRITICAL ERROR: {osm_file} not found. Please place the file there.")
            return
        ...
        cmd = [
            ...
            # No strict crop initially, relying on OSM file bounds
            "--proj.utm",
    ...
    def generate_routes(self):
        ...
        if not os.path.exists(self.net_file):
             print(f"Error: Net file {self.net_file} not found. Netconvert might have failed.")
             return
        ...
        if not valid_edges:
            print("Error: No valid edges found for passenger vehicles!")
            return
...
""")
    ...
    def run(self):
        ...
        print(f"\nSUCCESS! Katunayake scenario created at:\n{self.cfg_file}")

if __name__ == "__main__":
    KatunayakeScenarioGenerator().run()
